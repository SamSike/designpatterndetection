...
assert algorithm in ["RF", "SVM", "GBTREE", "ADABOOST",
                     "ADABOOST_LOGISTIC", "LOGISTIC", "RIDGE", "VOTER", "EXTRA_TREES"]
...
def train_test_get_metrics(model_builder, X_train, X_test, y_train, y_test):
    ...
    precision, recall, fscore, support = precision_recall_fscore_support(
        label_lookup[y_test], label_lookup[y_pred])
...
def adaboost_tree():
    # removed learning rate = 0.5
    return AdaBoostClassifier(base_estimator=RandomForestClassifier(n_estimators=100), n_estimators=20)
...
def summarise_cv_results(cv_results):
    cm, accuracy, balanced_accuracy, misclassification, precision, recall, fscore, support, precision_final, recall_final, fscore_final = list(
...
cm, accuracy, balanced_accuracy, misclassification, precision, recall, fscore, support, precision_final, recall_final, fscore_final = summarise_cv_results(
    cv_results)
...
df1 = pd.DataFrame({"Design Pattern": label_lookup,
                   "value": precision, "Metric": ["Precision"]*len(label_lookup)})
df2 = pd.DataFrame({"Design Pattern": label_lookup,
                   "value": recall, "Metric": ["Recall"] * len(label_lookup)})
...
ax1.set_title("Precision = %2.2f%%  Recall = %.2f%%" %
              (precision_final*100, recall_final*100))
