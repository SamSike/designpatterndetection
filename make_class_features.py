...
log.basicConfig(level=log.DEBUG, filename='make_class_features.log',
    ...
    for f in files:
        if ".verbose" in f:
            ...
            if class_dict is None:
                logger.warning(
                    "No classes or ngrams extracted from project file {0}".format(f))
                continue
...
for f, verbose_data in file_data_store.iteritems():
    ...
    for class_name, ngrams in class_dict.iteritems():
        ...
        feature_dict.update(
            {"w2v_"+str(i): x for i, x in enumerate(vector_ngram)})
...
print("-"*80)
print("Examples in Output-refined.csv but not in `.verbose` files = " +
      str(len(patterns_keys-saved_items_list)))
print("-"*80)
...
missing_projects = set(
    [proj for proj, class_name in sorted(patterns_keys-saved_items_list)])
...
patterns = pd.read_csv(data_file, header=None, names=[
                       "project_name", "class_name", "pattern"])
